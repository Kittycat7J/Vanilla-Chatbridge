...
# Record newest crash BEFORE starting
...
(
    while true; do
        ...
        if [[ -n "$NEW_CRASH" ]] && [[ "$NEW_CRASH" != "$BASELINE_CRASH" ]]; then
            ...
            # Graceful stop of the server only
...
# --- Crash handling (NO restart) ---
echo ">>> Server crashed — keeping bot alive for ${BOT_GRACE_SECONDS}s..."
sleep "$BOT_GRACE_SECONDS"

echo ">>> Grace period over, shutting down wrapper"
exit 0
